// IMPORTADOR COMPLETO COM ID PERSONALIZADO E TODOS OS CAMPOS
// Implementa: ID formato AA20253010104302 e mapeamento completo Excel → Access
...
async function importWithCustomID() {
  try {
    console.log('🚀 IMPORTADOR COMPLETO COM ID PERSONALIZADO');
    console.log('==========================================');
    ...
    console.log('\n📤 Iniciando importação com IDs personalizados...');
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        // Mapear booleanos
        const batizado = row.batizado === 'Sim' ? true : false;
        const membro = row.membro === 'Sim' ? true : false;
        const lider = row.e_lider === 'Sim' ? true : false;
        const professorEBQ = row['e_professor_ebq\n'] === 'Sim' ? true : false;
        const pequenoGrupo = row['Está em um pequeno grupo ?'] === 'Sim' ? true : false;
        ...
        const params = [
          ...
          row.nome,                                    // Nome
          row['Nome Completo'],                        // NomeCompleto
          dataNascimento,                              // DataNascimento
          idade,                                       // Idade calculada
          mes,                                         // Mes extraído
          sexo,                                        // Sexo (M/F)
          String(row.telefone || ''),                  // Telefone
          endereco,                                    // Endereco completo
          row.rua || '',                              // Rua
          String(row.numero || ''),                   // Numero
          row.bairro || '',                           // Bairro
          row.cidade || '',                           // Cidade
          row.estado || '',                           // Estado
          String(row.cep || ''),                      // CEP
          status,                                      // Status
          row.status_civil || '',                     // StatusCivil
          conjuge,                                     // Conjuge
          row['parentesco '] || '',                   // Parentesco
          batizado,                                    // Batizado
          membro,                                      // Membro
          lider,                                       // Lider
          professorEBQ,                               // ProfessorEBQ
          faixaEtaria,                                // FaixaEtaria calculada
          pequenoGrupo,                               // PequenoGrupo
          row.grupo || 'Sem Grupo',                   // Grupo
          row.numerodomes || null,                    // NumeroDomes
          timestamp.toISOString().replace('T', ' ').substring(0, 19), // DataCriacao
          timestamp.toISOString().replace('T', ' ').substring(0, 19)  // DataAtualizacao
        ];
