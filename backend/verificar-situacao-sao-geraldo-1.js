// Verificar situação_atual dos membros em São Geraldo 1
const { Pool } = require('pg');

const pool = new Pool({
  host: 'localhost',
  port: 5432,
  database: 'dashboard_membros',
  user: 'postgres',
  password: '252088'
});

async function verificarSituacao() {
  console.log('\n📊 VERIFICANDO SITUAÇÃO - SÃO GERALDO 1\n');
  console.log('═'.repeat(80));
  
  try {
    // Total geral
    const total = await pool.query(
      "SELECT COUNT(*) as total FROM membros WHERE bairro = 'São Geraldo 1'"
    );
    
    // Por situação_atual
    const porSituacao = await pool.query(`
      SELECT situacao_atual, COUNT(*) as total
      FROM membros
      WHERE bairro = 'São Geraldo 1'
      GROUP BY situacao_atual
      ORDER BY total DESC
    `);

    console.log(`Total de membros em São Geraldo 1: ${total.rows[0].total}\n`);
    console.log('Distribuição por situação:\n');
    
    porSituacao.rows.forEach(row => {
      const situacaoLabel = row.situacao_atual === 'Ativo' ? '✅ Ativos' : 
                           row.situacao_atual === 'Desligado' ? '❌ Desligados' : 
                           row.situacao_atual === 'Congregado' ? '🔵 Congregados' : 
                           `❓ ${row.situacao_atual || 'Sem situação'}`;
      console.log(`  ${situacaoLabel}: ${row.total}`);
    });

    console.log('\n' + '═'.repeat(80));
    console.log('\n💡 Os 19 membros incluem TODOS (ativos + desligados + outros)\n');

  } catch (error) {
    console.error('❌ Erro:', error.message);
    throw error;
  } finally {
    await pool.end();
  }
}

verificarSituacao().catch(err => {
  console.error('Erro fatal:', err);
  process.exit(1);
});
