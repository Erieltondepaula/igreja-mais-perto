...
export const MemberList = ({ members, onMemberUpdate }: MemberListProps) => {
    ...
    <Card className="rounded-xl shadow-md">
      ...
      <CardContent>
            ...
            <TableBody>
              {sortedMembers.map(member => (
                <TableRow key={member.id}>
                  ...
                  <TableCell><Badge variant="outline">{getMemberType(member)}</Badge></TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      <Button variant="ghost" size="icon" onClick={() => handleViewDetails(member)} className="rounded-full h-8 w-8">
        ...
        {sortedMembers.length === 0 && (
          <div className="text-center py-8 text-muted-foreground">Nenhum membro encontrado com os filtros aplicados.</div>
        )}
        ...
        <MemberEdit member={editingMember} isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} onSave={handleSaveEdit} />
      </CardContent>
    </Card>
  );
};
